...
Everyone interacting in the QAlgebra project's code base, issue tracker, and any communication channels is expected to follow the [PyPA Code of Conduct](https://www.pypa.io/en/latest/code-of-conduct/).

Report Bugs
-----------

Report bugs at <https://github.com/qalgebra/qalgebra/issues>.
...
The best way to send feedback is to file an issue at <https://github.com/qalgebra/qalgebra/issues>.
...
3.  Check <https://travis-ci.org/qalgebra/qalgebra/pull_requests> and make sure that the tests pass for all supported Python versions.

Get Started!
------------

Ready to contribute? Follow [Aaron Meurer's Git Workflow Notes](https://www.asmeurer.com/git-workflow/) (with `qalgebra/qalgebra` instead of `sympy/sympy`)

In short,

1.  Clone the repository from `git@github.com:qalgebra/qalgebra.git`
...
The project uses [tox](https://tox.readthedocs.io) for automated testing accross multiple versions of Python and for various development tasks such as linting and generating the documentation. See Development Prerequisites below for details.
...
For developers with direct access to the repository, QAlgebra uses a simple branching model where all developments happens directly on the `master` branch. Releases are tags on `master`. All commits on `master` *should* pass all tests and be well-documented. This is so that `git bisect` can be effective. For any non-trivial issue, it is recommended to create a topic branch, instead of working on `master`. There are no restrictions on commits on topic branches, they do not need to contain complete documentation, pass any tests, or even be able to run.
...
QAlgebra includes a full test-suite using [pytest](https://docs.pytest.org/en/latest/). We strive for a [test coverage](https://coveralls.io/github/qalgebra/qalgebra?branch=master) above 90%.

From a checkout of the `qalgebra` repository you can use
...
The tests are organized in the `tests` subfolder. It includes python scripts whose name start with `test_`, which contain functions whose names also start with `test_`. Any such functions in any such files are picked up by [pytest](https://docs.pytest.org/en/latest/) for testing. In addition, [doctests](https://docs.python.org/3.7/library/doctest.html) from any docstring or any documentation file (`*.rst`) are picked up (by the [pytest doctest plugin](https://docs.pytest.org/en/latest/doctest.html)). Lastly, all Jupyter notebooks in the documentation are validated as a test, through the [nbval plugin](https://nbval.readthedocs.io/en/latest/).

Code Style
----------

All code must be compatible with `8`. The line length limit is 79 characters, although exceptions are permissible if this improves readability significantly.

Beyond `8`, this project adopts the [Black code style](https://github.com/ambv/black/#the-black-code-style), with `--skip-string-normalization --line-length 79`. You can run `make black-check` or `tox -e run-blackcheck` to check adherence to the code style, and `make black` or `tox -e run-black` to apply it.
...
QAlgebra could always use more documentation, whether as part of the official docs, in docstrings, or even on the web in blog posts, articles, and such.
...
At any point, from a checkout of the `qalgebra` repository, you may run
...
Releases should follow [Semantic Versioning](https://semver.org), and version numbers published to [PyPI](http://pypi.org) must be compatible with `440`.

In short, versions number follow the pattern <span class="title-ref">major.minor.patch</span>, e.g. `0.1.0` for the first release, and `1.0.0` for the first *stable* release. If necessary, pre-release versions might be published as e.g:
...
The current version is available through the `__version__` attribute of the `qalgebra` package:

~~~ pycon
>>> import qalgebra
>>> qalgebra.__version__  # doctest: +SKIP
~~~
...
Releases must be tagged in git, using the version string prefixed by "v", e.g. `v1.0.0-dev1` and `v1.0.0`. This makes them available at <https://github.com/qalgebra/qalgebra/releases>.
