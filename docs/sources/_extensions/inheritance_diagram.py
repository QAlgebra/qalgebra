...
class InheritanceGraph(object):
    ...
    def __init__(
        ...
        if not self.class_info:
            raise InheritanceException(
                'No classes found for ' 'inheritance diagram'
            )
        ...
        *graph_attrs*, *node_attrs*, *edge_attrs* are dictionaries containing
        ...
        if self.cluster_modules:
            for subgraph_name in subgraphs:
                ...
                res.append('  label="%s";\n' % subgraph_name)
                res.append(
                    '  graph[' + self._format_node_attrs(c_attrs) + "];\n"
                )
...
def html_visit_inheritance_diagram(self, node):
    ...
    graphviz_output_format = (
        self.builder.env.config.graphviz_output_format.upper()
    )
    ...
    for child in node:
        if child.get('refuri') is not None:
            if graphviz_output_format == 'SVG':
                urls[child['reftitle']] = os.path.join(
                    "..", child.get('refuri')
                )
            else:
    ...
    render_dot_html(
        ...
        link_to_svg='<i class="fa fa-external-link" aria-hidden="true"></i>'
        ' SVG',
    )
