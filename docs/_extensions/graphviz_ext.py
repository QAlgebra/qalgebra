...
class ClickableMapDefinition(object):
    ...
    def parse(self, dot=None):
        ...
        if not matched:
            raise GraphvizError(
                'Invalid clickable map file found: %s' % self.filename
            )
    ...
    def generate_clickable_map(self):
        ...
        if self.clickable:
            return '\n'.join(
                [self.content[0]] + self.clickable + [self.content[-1]]
            )
        else:
            return ''
...
def render_dot_html(
    ...
    if fname is None:
        ...
        if format == 'svg':
            ...
            self.body.append(
                '<object data="%s" type="image/svg+xml" %s>\n'
                % (fname, imgcss)
            )
            ...
            with codecs.open(outfn + '.map', 'r', encoding='utf-8') as mapfile:  # type: ignore
                ...
                if imgmap.clickable:
                    ...
                    self.body.append('<div class="graphviz">')
                    self.body.append(
                        '<img src="%s" alt="%s" %s/>' % (fname, alt, imgcss)
                    )
                    self.body.append('</div>\n')
        if 'align' in node:
            self.body.append('</div>\n')
